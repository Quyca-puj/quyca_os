...
void setSpeedsMotor(int leftSp,int rigthSp){
  //***********************************************************************************
      ...
      if (leftSp >= 0)
      {
        ...
        leftS = config.get(ConfigOptions::SPEED_OFFSET_LEFT) - map(leftSp, 0, 255, 0, 90);
      }
      else
      {
        // Scale it to use it with the servo (value between 0 and 90 -> 90 to 180).
        leftS = map( (leftSp * -1) , 0, 255, 0, 90) + config.get(ConfigOptions::SPEED_OFFSET_LEFT);
      }
      ...
      {
        // Scale it to use it with the servo (value between 0 and 90 -> 90 to 180).
        rigthS =  map(rigthSp, 0, 255, 0, 90) + config.get(ConfigOptions::SPEED_OFFSET_RIGHT);
      }
      else
      {
        // Scale it to use it with the servo (value between 0 and 90 -> 0 to 90).
        rigthS = config.get(ConfigOptions::SPEED_OFFSET_RIGHT) - map( (rigthSp * -1) , 0, 255, 0, 90);
      }
